...
df2 <- df %>%
  select(
    date, pray, meditate, cardio, strength, post_cfm_study, post_gratitude, sun,sss
  ) %>% 
...
df2_recent$Day <- format(as.Date(df2_recent$Day), "%a %m/%e")
...
df2_recent %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(2:9)  # replace with the actual column indices or names
    )
  ) %>%
